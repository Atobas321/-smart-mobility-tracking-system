...
if "🗺" in page:
    ...
    with col_side:
        ...
        ICONS = {"car": "🚗", "moto": "🏍", "trike": "🛺"}
        selected_id = None
        for _, row in df.iterrows():
            veh_icon = ICONS.get(row["type"], "🚗")
            with st.container():
                if st.button(
                    f"{veh_icon} {row['id']}  ·  {row['city']}",
        ...
        if selected_id:
            ...
            if not sel.empty:
                row = sel.iloc[0]
                sel_icon = ICONS.get(row["type"], "🚗")
                speed_color = "#ff4d6a" if row["speed"] > 80 else "#22e5a0"
                ...
                <div style='background:#161b27;border:1px solid #21293d;border-radius:8px;padding:12px;font-size:12px'>
                  ...
                  <div style='color:#8895bc;line-height:2'>
                    ...
                    Speed: <b style='color:{speed_color}'>{row['speed']} km/h</b><br>
